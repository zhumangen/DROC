...
void GraphicsCobbAngleItem::resizeItem()
{
    line1Vertex1->setScale(zoomFactor);
    line1Vertex2->setScale(zoomFactor);
    line2Vertex1->setScale(zoomFactor);
    line2Vertex2->setScale(zoomFactor);
    auxVertex->setScale(zoomFactor);
}
...
QPointF GraphicsCobbAngleItem::textItemPos()
{
    ...
    if ((vector1.x() < 0 || vector1.y() < 0) && (vector2.x() < 0 || vector2.y() < 0)) {
        if (angleCacheData <= 90.0)
            return QPointF(line1Vertex1->pos().x() + line1Vertex1->crossSize().width()*zoomFactor,
                           line1Vertex1->pos().y());
        else
            return QPointF(line1Vertex2->pos().x() + line1Vertex2->crossSize().width()*zoomFactor,
                           line1Vertex2->pos().y());
    } else if ((vector1.x() < 0 || vector1.y() > 0) && (vector2.x() < 0 || vector2.y() > 0)) {
        if (angleCacheData <= 90.0)
            return QPointF(line1Vertex1->pos().x() + line1Vertex1->crossSize().width()*zoomFactor,
                           line1Vertex1->pos().y() - textItem->boundingRect().height()*zoomFactor);
        else
            return QPointF(line1Vertex2->pos().x() + line1Vertex2->crossSize().width()*zoomFactor,
                           line1Vertex2->pos().y() - textItem->boundingRect().height()*zoomFactor);
    } else {
        if (angleCacheData <= 90.0)
            return QPointF(line1Vertex1->pos().x() -
                           (line1Vertex1->crossSize().width() + textItem->boundingRect().width())*zoomFactor,
                           line1Vertex1->y());
        else
            return QPointF(line1Vertex2->pos().x() -
                           (line1Vertex2->crossSize().width() + textItem->boundingRect().width())*zoomFactor,
                           line1Vertex2->y());
    }
}
